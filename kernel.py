...
#функция для преобразования перепада давления к диапазону от 1 и больше (иногда задается диапазоном от 0 до 1)
def func_reverse(x):
    return (1/x)

#функция для конвертации единиц при необходимости
def func_unit_convert(x):
    return x

#функция для преобразования кпд к диапазону от 0 до 1 (иногда задается диапазоном от 0 до 100)
def func_eff_convert(x):
    return x/100
...
def f_factory(func, val,
              val2=0):  # вспомогательная фабрика функций (https://stackoverflow.com/questions/3431676/creating-functions-in-a-loop)
...
class approximator():
    def __init__(self, type, name_of_file, print_plot=True):
        ...
        self.date_text = self.now.strftime("%Y-%m-%d %H_%M")
        # 0) выбор типа характеристики: турбина или компрессор
        self.type_of_map = type
        # self.name_of_file = "2020 11 аэродинамические характеристики компрессора ГТЭ-170 по результатам CFD\AC CFD 2020 11 A-3.csv"  # !!!указать файл
        self.name_of_file =  name_of_file
        # 1)импорт данных
        with open(os.getcwd() + self.name_of_file) as initial_data:
            self.reader = pd.read_csv(initial_data, delimiter=';', decimal=',')
        # 1.2)проверяем, чтобы степени повышения/понижения давления не были указаны в диапазоне от 0 до 1
        if self.reader.PR[0] < 1:
            self.reader['PR'] = self.reader['PR'].apply(func_reverse)
        # 1.21)!!! проверяем чтобы все единицы были в СИ, при необходимости корректируем вручную здесь
        self.reader['G']=self.reader['G'].apply(func_unit_convert)
        # 1.22)!!! КПД д.б. в диапазоне от 0 до 1
        if any(self.reader['Eff'])>1:
            self.reader['Eff']=self.reader['Eff'].apply(func_eff_convert)
        # 1.3) добавляем параметр k, по которому дальше будем сортировать
        if self.type_of_map == "turbine":
            self.reader['k'] = self.reader.PR
            self.reader['Gn'] = self.reader.G * self.reader.n
        if self.type_of_map == "compressor":
            self.reader['k'] = self.reader.PR / self.reader.G
        ...
        self.reader.sort_values(['n', 'k', 'G'], ascending=[True, True, False], inplace=True)
        self.reader = self.reader.reset_index(drop=True)
        ...
        self.rotations = pd.unique(self.reader.n)
        self.DATA['rotations'] = self.rotations
        ...
        LowCurveX = []
        LowCurveY = []
        for nx in self.rotations:
            temp_data = self.reader.loc[self.reader.n == nx]
            if self.type_of_map == "turbine":
        ...
        HighCurve = np.poly1d(np.polyfit(HighCurveX, HighCurveY,
                                         2))  # NB! тк кривая близка к вертикальной, то здесь удобно поменять местами x и y и соответственнно дальнейшее использование этих функций
        ...
        Xlowmin = LowCurveX[0];
        Xlowmax = LowCurveX[-1];
        Xlowmid = (Xlowmin + Xlowmax) / 2
        Ylowmin = LowCurveY[0];
        Ylowmax = LowCurveY[-1];
        Ylowmid = (Ylowmin + Ylowmax) / 2
        Xhighmin = HighCurveX[0];
        Xhighmax = HighCurveX[-1];
        Xhighmid = (Xhighmin + Xhighmax) / 2
        Yhighmin = HighCurveY[0];
        Yhighmax = HighCurveY[-1];
        Yhighmid = (Yhighmin + Yhighmax) / 2
        ...
        self.Xlowmincrv, self.Ylowmincrv = find_pnt(LowCurve, [Xlowmin, Ylowmin])
        self.Xlowmaxcrv, self.Ylowmaxcrv = find_pnt(LowCurve, [Xlowmax, Ylowmax])
        self.Xlowmidcrv, self.Ylowmidcrv = find_pnt(LowCurve, [Xlowmid, Ylowmid])
        ...
        self.Xhighmincrv, self.Yhighmincrv = find_pnt(HighCurve, [Xhighmin, Yhighmin])
        self.Xhighmaxcrv, self.Yhighmaxcrv = find_pnt(HighCurve, [Xhighmax, Yhighmax])
        self.Xhighmidcrv, self.Yhighmidcrv = find_pnt(HighCurve, [Xhighmid, Yhighmid])
        ...
        if self.type_of_map == "turbine":
            points_for_scatter = [{'x': self.reader.PRrel, 'y': self.reader.Gnrel, 'label': 'исходные данные', 's': 1},
                                  {'x': HighCurveX, 'y': HighCurveY, 'label': 'верхние крайние точки', 's': 10,
                                   'marker': 'x', 'c': 'red'},
                                  {'x': LowCurveX, 'y': LowCurveY, 'label': 'нижние крайние точки', 's': 10,
                                   'marker': 'x', 'c': 'blue'},
                                  {'x': [Xlowmin, Xlowmid, Xlowmax], 'y': [Ylowmin, Ylowmid, Ylowmax],
                                   'label': 'нижние опорные точки', 's': 20, 'marker': 'o', 'c': 'black'},
                                  {'x': [self.Xlowmincrv, self.Xlowmidcrv, self.Xlowmaxcrv], 'y': [self.Ylowmincrv, self.Ylowmidcrv, self.Ylowmaxcrv],
                                   'label': 'нижние опорные точки на кривой', 's': 30, 'marker': '+', 'c': 'black'},
                                  {'x': [Xhighmin, Xhighmid, Xhighmax], 'y': [Yhighmin, Yhighmid, Yhighmax],
                                   'label': 'верхние опорные точки', 's': 20, 'marker': 'o', 'c': 'black'},
                                  {'x': [self.Xhighmincrv, self.Xhighmidcrv, self.Xhighmaxcrv],
                                   'y': [self.Yhighmincrv, self.Yhighmidcrv, self.Yhighmaxcrv],
                                   'label': 'верхние опорные точки на кривой', 's': 30, 'marker': '+', 'c': 'black'}]
            # points_for_plot=[]
            points_for_plot = [{'x': x_high, 'y': y},
                               {'x': x_low, 'y': y}, ]

            x_label_caption='PR relative'
            y_label_caption = 'G*n relative'
        elif self.type_of_map == "compressor":
            points_for_scatter = [{'x': self.reader.Grel, 'y': self.reader.PRrel, 'label': 'исходные данные', 's': 1},
                                  {'x': HighCurveX, 'y': HighCurveY, 'label': 'верхние крайние точки', 's': 10,
                                   'marker': 'x', 'c': 'red'},
                                  {'x': LowCurveX, 'y': LowCurveY, 'label': 'нижние крайние точки', 's': 10,
                                   'marker': 'x', 'c': 'blue'},
    ...
    def BettaCurve(self, betta):
        Ybettamin=self.Ylowmincrv+betta*(self.Yhighmincrv-self.Ylowmincrv)
        Ybettamid=self.Ylowmidcrv+betta*(self.Yhighmidcrv-self.Ylowmidcrv)
        Ybettamax=self.Ylowmaxcrv+betta*(self.Yhighmaxcrv-self.Ylowmaxcrv)
        Xbettamin=self.Xlowmincrv+betta*(self.Xhighmincrv-self.Xlowmincrv)
        Xbettamid=self.Xlowmidcrv+betta*(self.Xhighmidcrv-self.Xlowmidcrv)
        Xbettamax=self.Xlowmaxcrv+betta*(self.Xhighmaxcrv-self.Xlowmaxcrv)
    ...
    def stage1_extrapolation(self, print_plot=True):
        ...
        self.reader['betta']=np.nan
        for ni in self.rotations:
            temp_data=self.reader.loc[self.reader.n == ni] #выделяем из общего массива данных только те, что относятся к определенным оборотам nx
            G_temp=temp_data.G
            ...
            if self.type_of_map=="turbine":
                for XY in temp_data.loc[:,['PRrel','Gnrel']].values:
                    X_PR=XY[0];Y_Gn=XY[1]
                    _func=lambda betta: self.BettaCurve(betta)(Y_Gn)-X_PR
                    _betta=root(_func,x0=0.5)['x'][0]
                    Betta_temp.append(_betta)
            elif self.type_of_map == "compressor":
                for XY in temp_data.loc[:,['PRrel','Grel']].values:
                    X_G=XY[1];Y_PR=XY[0]
                    _func=lambda betta: self.BettaCurve(betta)(X_G)-Y_PR
                    _betta=root(_func,x0=0.5)['x'][0]
                    Betta_temp.append(_betta)
            ...
            if "initial_reader" not in locals():
                initial_reader = self.reader.copy()
                # reader['betta']=np.nan
            self.reader.update(temp_data)

        self.DATA['initial_reader']=initial_reader

        #строим регрессию на основе значений угловых коэффициентов и значений оборотов
        _rotations=np.concatenate([self.rotations[0]*0.9,self.rotations[0]*0.933,self.rotations[0]*0.966,self.rotations,self.rotations[-1]*1.033,self.rotations[-1]*1.066,self.rotations[-1]*1.1],axis=None)
        _rotations2=np.concatenate([self.rotations,self.rotations[-1]*1.033,self.rotations[-1]*1.066,self.rotations[-1]*1.1],axis=None)
        ...
        self.slopes_G_low_func=Akima1DInterpolatorModified(x=_rotations,y=np.concatenate([slopes_G_low[0],slopes_G_low[0],slopes_G_low[0],slopes_G_low,slopes_G_low[-1],slopes_G_low[-1],slopes_G_low[-1]],axis=None))
        self.slopes_PR_low_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_PR_low[0],slopes_PR_low[0],slopes_PR_low[0],slopes_PR_low,slopes_PR_low[-1],slopes_PR_low[-1],slopes_PR_low[-1]],axis=None),deg=3))
        self.slopes_Eff_low_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_Eff_low[0],slopes_Eff_low[0],slopes_Eff_low[0],slopes_Eff_low,slopes_Eff_low[-1],slopes_Eff_low[-1],slopes_Eff_low[-1]],axis=None),deg=3))
        ...
        self.slopes_G_high_func=np.poly1d(np.polyfit(x=_rotations2,y=np.concatenate([slopes_G_high,slopes_G_high[-1],slopes_G_high[-1],slopes_G_high[-1]],axis=None),deg=4))
        ...
        # slopes_G_high_func = Akima1DInterpolatorModified(x=_rotations2, y=np.concatenate([slopes_G_high, slopes_G_high[-1], slopes_G_high[-1], slopes_G_high[-1]], axis=None))
        # slopes_PR_high_func = np.poly1d(np.polyfit(x=_rotations, y=np.concatenate(
        #     [slopes_PR_high[0], slopes_PR_high[0], slopes_PR_high[0], slopes_PR_high, slopes_PR_high[-1],
        #      slopes_PR_high[-1], slopes_PR_high[-1]], axis=None), deg=3))
        self.slopes_PR_high_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_PR_high[0],slopes_PR_high[0],slopes_PR_high[0],slopes_PR_high,slopes_PR_high[-1],slopes_PR_high[-1],slopes_PR_high[-1]],axis=None),deg=3))
        self.slopes_Eff_high_func=np.poly1d(np.polyfit(x=_rotations,y=np.concatenate([slopes_Eff_high[0],slopes_Eff_high[0],slopes_Eff_high[0],slopes_Eff_high,slopes_Eff_high[-1],slopes_Eff_high[-1],slopes_Eff_high[-1]],axis=None),deg=3))
        ...
        if self.type_of_map=="turbine":
            self.slopes_Gn_low_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_Gn_low,deg=3))
            self.slopes_Gn_high_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_Gn_high,deg=3))
            if "L" in temp_data:
                self.slopes_L_low_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_L_low,deg=3))
                self.slopes_L_high_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_L_high,deg=3))
            if "A" in temp_data:
                self.slopes_A_low_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_A_low,deg=3))
                self.slopes_A_high_func=np.poly1d(np.polyfit(x=self.rotations,y=slopes_A_high,deg=3))
        ...
        if self.type_of_map=="compressor":
            ...
            _slopes_G_low=[self.slopes_G_low_func(n) for n in _rotations]
            _slopes_G_high=[self.slopes_G_high_func(n) for n in _rotations]
            _slopes_PR_low=[self.slopes_PR_low_func(n) for n in _rotations]
            _slopes_PR_high=[self.slopes_PR_high_func(n) for n in _rotations]
            _slopes_Eff_low=[self.slopes_Eff_low_func(n) for n in _rotations]
            _slopes_Eff_high=[self.slopes_Eff_high_func(n) for n in _rotations]

            points_for_scatter=[{'x':self.rotations,'y':slopes_G_low,'label':f'G_low', 's': 30, 'marker': '+'},
                                {'x':self.rotations,'y':slopes_G_high,'label':f'G_high', 's': 30, 'marker': '+'},
                                {'x':self.rotations,'y':slopes_PR_low,'label':f'PR_low', 's': 30, 'marker': '+'},
                                {'x':self.rotations,'y':slopes_PR_high,'label':f'PR_high', 's': 30, 'marker': '+'},
                                {'x':self.rotations,'y':slopes_Eff_low,'label':f'Eff_low', 's': 30, 'marker': '+'},
                                {'x':self.rotations,'y':slopes_Eff_high,'label':f'Eff_high', 's': 30, 'marker': '+'}]
            points_for_plot=[{'x':_rotations,'y':_slopes_G_low,'label':'G_low'},
                                {'x':_rotations,'y':_slopes_G_high,'label':'G_high'},
                                {'x':_rotations,'y':_slopes_PR_low,'label':'PR_low'},
        ...
        if self.type_of_map=="turbine":
            ...
            _slopes_G_low=[self.slopes_G_low_func(n) for n in _rotations]
            _slopes_G_high=[self.slopes_G_high_func(n) for n in _rotations]
            _slopes_Gn_low=[self.slopes_Gn_low_func(n) for n in _rotations]
            _slopes_Gn_high=[self.slopes_Gn_high_func(n) for n in _rotations]
            _slopes_PR_low=[self.slopes_PR_low_func(n) for n in _rotations]
            _slopes_PR_high=[self.slopes_PR_high_func(n) for n in _rotations]
            _slopes_Eff_low=[self.slopes_Eff_low_func(n) for n in _rotations]
            _slopes_Eff_high=[self.slopes_Eff_high_func(n) for n in _rotations]

            points_for_scatter=[{'x':self.rotations,'y':slopes_G_low,'label':'slopes_G_low'},
                                {'x':self.rotations,'y':slopes_G_high,'label':'slopes_G_high'},
                                {'x':self.rotations,'y':slopes_Gn_low,'label':'slopes_Gn_low'},
                                {'x':self.rotations,'y':slopes_Gn_high,'label':'slopes_Gn_high'},
                                {'x':self.rotations,'y':slopes_PR_low,'label':'slopes_PR_low'},
                                {'x':self.rotations,'y':slopes_PR_high,'label':'slopes_PR_high'},]
            points_for_scatter_Eff=[{'x':self.rotations,'y':slopes_Eff_low,'label':'slopes_Eff_low'},
                                {'x':self.rotations,'y':slopes_Eff_high,'label':'slopes_Eff_high'}]
    ...
    def stage2_check_interpolation_along_n_curve_const(self, print_plot=True):
        ...
        for ni in self.rotations:
            ...
            slope_G_low=self.slopes_G_low_func(ni)
            slope_PR_low=self.slopes_PR_low_func(ni)
            slope_Eff_low=self.slopes_Eff_low_func(ni)
            slope_G_high=self.slopes_G_high_func(ni)
            slope_PR_high=self.slopes_PR_high_func(ni)
            slope_Eff_high=self.slopes_Eff_high_func(ni)
            ...
            if self.type_of_map=="turbine":
                slope_Gn_low=self.slopes_Gn_low_func(ni)
                slope_Gn_high=self.slopes_Gn_high_func(ni)
                ...
                if "L" in temp_data:
                    slope_L_low=self.slopes_L_low_func(ni)
                    slope_L_high=self.slopes_L_high_func(ni)
                    intercept_L_low=L_temp.iloc[0]-slope_L_low*betta_temp.iloc[0]
                    intercept_L_high=L_temp.iloc[-1]-slope_L_high*betta_temp.iloc[-1]
                if "A" in temp_data:
                    slope_A_low=self.slopes_A_low_func(ni)
                    slope_A_high=self.slopes_A_high_func(ni)
            ...
            if 'new_reader' not in locals():
                ...
                new_reader = pd.concat([new_reader, _temp], ignore_index=True)

        self.DATA['new_reader']=new_reader
        self.DATA['rel_maximums']=rel_maximums
        ...
        for ind,ni_color in enumerate(zip(self.rotations,colors)):#проходимся по всем заданным веткам оборотов, строим графики для проверки
            ni=ni_color[0]
            color=ni_color[1]
            ...
            Eff_func_rel=UnivariateSplineModified(Betta_temp,Eff_temp,k=3,s=0.0005)#!!! Здесь нужно правильно выбрать аппроксимирующую функцию, точная сплайн-интерполяция хороша только если исходные данные обладают высокой точностью, иначе лучше использовать аппроксимацию
